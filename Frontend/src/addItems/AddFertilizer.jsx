...
function AddFertilizer(){
    ...
    const [formdata, setFormdata] = useState({fertilizerCategory: '',name: '', description: '', price: '', quantity: '', photo: '', state: '', district: '', place: ''})
    ...
    const fertilizerCategories = ["Urea", "DAP", "Phpsphorus", "Sulphur", "Organic", "Other"]
    ...
    const handleSubmit = async (e) => {
        ...
        await fetch(`${backendUrl}/add/fertilizer/${user._id}`, {
            method: 'POST',
            body: formData
        }).then(async (res) => {
            ...
            if(res.ok){
                ...
                setFormdata({fertilizerCategory: '',name: '', description: '', price: '', quantity: '', photo: '', state: '', district: '', place: ''});
            }else{
    ...
    return (
        <div className="bg-zinc-800 min-h-screen text-white flex justify-center items-center py-4 sm:py-10">
            <div className="p-5 border-2 border-gray-400 rounded-lg w-[80%] lg:w-[40%]">
                <h1 className="font-extrabold text-[25px] text-gray-500 py-4">Add Fertilizer</h1>
                <form onSubmit={handleSubmit} className="flex flex-col">

                    <label htmlFor="category" className="py-2 mt-4">Category <span className="text-red-500">*</span></label>
                    <select name="fertilizerCategory" id="category" required value={formdata.fertilizerCategory} onChange={handleChange} className="px-3 py-2 bg-transparent text-white border-2 border-zinc-600 rounded-lg outline-none">
                        <option value="" className="text-black">Select--</option>
                        {fertilizerCategories.map((m) => (
                            <option key={m} value={m} className="px-4 py-1 text-black">{m}</option>
                        ))}
                    </select>

                    <label htmlFor="name" className="py-2">Name of Fertilizer <span className="text-red-500">*</span></label>
                    <input type="text" id="name" name="name" required placeholder="eg: Urea/NPK" value={formdata.name} onChange={handleChange} className="border-2 border-gray-500 bg-transparent outline-none text-white px-3 py-2 rounded-lg" />

                    <label htmlFor="description" className="py-2 mt-4">Description</label>
                    <textarea id="description" name="description" placeholder="eg: contact till monday" value={formdata.description} onChange={handleChange} className="border-2 border-gray-500 bg-transparent outline-none text-white px-3 py-2 rounded-lg"></textarea>

                    <label htmlFor="price" className="py-2 mt-4">Price <span className="text-red-500">*</span></label>
                    <input type="text" id="price" name="price" required placeholder="eg: 2400 per quintal" value={formdata.price} onChange={handleChange} className="border-2 border-gray-500 bg-transparent outline-none text-white px-3 py-2 rounded-lg" />

                    <label htmlFor="quantity" className="py-2">Quantity of Fertilizer</label>
                    <input type="text" id="quantity" name="quantity" placeholder="eg: 5 Quintal" value={formdata.quantity} onChange={handleChange} className="border-2 border-gray-500 bg-transparent outline-none text-white px-3 py-2 rounded-lg" />

                    <label htmlFor="image" className="py-2 mt-4">Photo of Fertilizer<span className="text-red-500">*</span></label>
