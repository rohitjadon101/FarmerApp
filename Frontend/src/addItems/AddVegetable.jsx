...
function AddVegetable(){
    ...
    const [formdata, setFormdata] = useState({vegetableCategory: '',name: '', description: '', price: '', quantity: '', photo: '', state: '', district: '', place: ''})
    ...
    const vegetableCategories = ["Potato", "Tomato", "Onion", "Ginger", "Carrot", "Other"]
    ...
    const handleSubmit = async (e) => {
        ...
        await fetch(`${backendUrl}/add/vegetable/${user._id}`, {
            method: 'POST',
            body: formData
        }).then(async (res) => {
            ...
            if(res.ok){
                ...
                setFormdata({vegetableCategory: '',name: '', description: '', price: '', quantity: '', photo: '', state: '', district: '', place: ''});
            }else{
    ...
    return (
        <div className="bg-zinc-800 min-h-screen text-white flex justify-center items-center py-4 sm:py-10">
            <div className="p-5 border-2 border-gray-400 rounded-lg w-[80%] lg:w-[40%]">
                <h1 className="font-extrabold text-[25px] text-gray-500 py-4">Add Vegetable</h1>
                <form onSubmit={handleSubmit} className="flex flex-col">

                    <label htmlFor="category" className="py-2 mt-4">Category <span className="text-red-500">*</span></label>
                    <select name="vegetableCategory" id="category" required value={formdata.vegetableCategory} onChange={handleChange} className="px-3 py-2 bg-transparent text-white border-2 border-zinc-600 rounded-lg outline-none">
                        <option value="" className="text-black">Select--</option>
                        {vegetableCategories.map((m) => (
                            <option key={m} value={m} className="px-4 py-1 text-black">{m}</option>
                        ))}
                    </select>

                    <label htmlFor="name" className="py-2">Name of Vegetable <span className="text-red-500">*</span></label>
                    <input type="text" id="name" name="name" required placeholder="eg: Onion" value={formdata.name} onChange={handleChange} className="border-2 border-gray-500 bg-transparent outline-none text-white px-3 py-2 rounded-lg" />
                    ...
                    <input type="text" id="quantity" name="quantity" placeholder="eg: 2 Quintal" value={formdata.quantity} onChange={handleChange} className="border-2 border-gray-500 bg-transparent outline-none text-white px-3 py-2 rounded-lg" />

                    <label htmlFor="image" className="py-2 mt-4">Photo of Vegetable<span className="text-red-500">*</span></label>
